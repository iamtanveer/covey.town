...
/**
 * Payload sent by client to create a private channel
 */
...
export async function townJoinHandler(
  ...
  assert(newSession.groupChatChannelSId);
  return {
    isOK: true,
    response: {
      ...
      groupChatChannelSID: newSession.groupChatChannelSId,
    },
  };
}
...
export async function createPrivateChannel(requestData:CreatePrivateChannelRequest) : Promise<ResponseEnvelope<CreateChannelResponse>> {
  ...
  const newChannelSid = await coveyTownController.createChannel(requestData.requestorUserID);
